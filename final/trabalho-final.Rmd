...
**Resposta:** Podemos gerar várias filas e obter vários possíveis tamanhos máximos de fila:

```{r Calculando tempo médio de atendimento e chegada}
simulate.people.waiting <- function(samples.count, sample.size, results, servers) {
  people.waiting = numeric(samples.count*nrow(results))
  count = 1
  for (i in 1:samples.count) {
    for (j in 1:nrow(results)) {
      result <- results[i,]
      queue.obj <- queue.creator(sample.size, result$lambda, result$mu, servers)
      people.waiting[count] <- max(queue.obj$queuelength_df$queuelength)
      count <- count+1
    }
  }
  people.waiting
...
people.waiting <- simulate.people.waiting(simulation.per.point.count, sample.size, best.results, 2)
```
...
Assim, por volta de 90 cadeiras deve ser suficiente para garantir que os pacientes estariam sentados 9 em cada 10 vezes.